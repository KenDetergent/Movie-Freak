...
export default function App() {
  ...
  function handleAddWatched(movie) {
    // ...Arrayでその配列のすべての要素を新しい配列にコピー 　,Objectで配列末尾に追加
    ...
    function () {
      async function fetchMovies() {
        try {
          ...
          // response objectのプロパティok(boolean)の値をチェックする。Http status code = 200~299 ならTrue
          if (!res.ok) {
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  // watchedはAdd to Listボタンを押したときに、現在の選択しているMovieのオブジェクトとして入る
  const isWatched = watched.map((movie) => movie.imdbID).includes(selectedId);
  console.log(isWatched);

  // stateのmovieは下のuseEffectのところでfetchした詳細APIのデータが入っている
  // constの形で複数の変数を一気に定義している
  const {
    Title: title,
    Year: year,
    Rated: rated,
  ...
  function handleAdd() {
    // 上で定義した詳細APIのデータを入れた新オブジェクトを作成する
